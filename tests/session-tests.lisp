...
(defparameter *retrieve-session-json*
  "{
...
(define-mock-query mock-retrieve-session (:type stripe:session)
  *retrieve-session-json*)

(test retrieve-session
  (let ((session (mock-retrieve-session)))
    (is (typep session 'stripe:session))
    (is (string= (stripe:id session)
